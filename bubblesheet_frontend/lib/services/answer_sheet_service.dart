...
class AnswerSheetService {
  static Future<List<AnswerSheet>> getAnswerSheets(String? token) async {
    print('Calling API: ${ApiService.baseUrl}/answer-sheets/');
    ...
    if (response.statusCode == 200) {
      ...
      // Lấy từ 'results' nếu có, nếu không thì fallback sang body luôn (trường hợp không dùng pagination)
      final List<dynamic> data = body is Map && body.containsKey('results')
          ? body['results']
          : (body is List ? body : []);
      print('Parsed data: $data');
      
      final sheets = data.map((e) => AnswerSheet.fromJson(e)).toList();
      print('Converted to AnswerSheet objects: ${sheets.length}');
      return sheets;
    } else {
